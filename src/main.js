...
import { createRenderer, createScene, createCamera, createComposer, bindResizeHandler } from "./aquarium/sceneSetup.js";
import { buildAquarium, setupPlankton, updatePlankton } from "./aquarium/builder.js";
...
const config = { ...CONFIG, lights: CONFIG.lights.map((l) => ({ ...l })), ...(saved ? { ...saved, lights: undefined } : {}) };
...
const ctrl = new GizmoController(scene, config, { hallOfFame, hofStats, gizmoList, selectionRules });
...
// ── Camera zoom state ─────────────────────────────────────────────────────────
let zoomCurrent = 1.0, zoomTarget = 1.0, zoomTargetX = 0, zoomTargetY = 0;
...
window.addEventListener("keydown", (e) => {
  if (e.code === "Space" && e.target === document.body) { e.preventDefault(); paused = !paused; }
});
...
renderer.domElement.addEventListener("click", (e) => {
  ...
  const wx = (((e.clientX - rect.left) / rect.width) * 2 - 1) * (window.innerWidth / 2);
  const wy = -(((e.clientY - rect.top) / rect.height) * 2 - 1) * (window.innerHeight / 2);

  let closest = null, closestDist = Infinity;
  ctrl.gizmos.forEach((g) => {
    const dist = Math.sqrt((g.position.x - wx) ** 2 + (g.position.y - wy) ** 2);
    if (dist <= (g.bodyRadius ?? 15) && dist < closestDist) { closest = g; closestDist = dist; }
  });
  if (closest) {
...
const gizmosRef = { get current() { return ctrl.gizmos; }, set current(v) { ctrl.gizmos = v; } };
const { openAquariumControls } = setupGui(gui, config, {
  foodManager: { reset: () => foodManager?.reset() }, foodStats, hofStats,
  createGizmos: () => ctrl.createGizmos(),
  hallOfFame, gizmosRef,
  spawnGeneration: (...a) => ctrl.spawnGeneration(...a),
  openSettingsModal, AQUARIUM_PRESETS,
  buildAquariumFn: () => buildAquarium(scene, config, aqObjects),
  saveConfig, clearSelection: () => ctrl.clearSelection(), STORAGE_KEY,
  startSimulation,
});
...
(function animate() {
  ...
  if (paused || !simulationStarted) { composer.render(); return; }
  ...
  zoomTarget = state.zoom; zoomTargetX = state.targetX; zoomTargetY = state.targetY;
  zoomCurrent = Math.abs(zoomCurrent - zoomTarget) > 0.01
    ? zoomCurrent + (zoomTarget - zoomCurrent) * zoomSpeed * dt
    : zoomTarget;
  if (zoomTarget === 1.0) { zoomTargetX *= (1 - 3 * dt); zoomTargetY *= (1 - 3 * dt); }
  const hw = originalCameraWidth / 2 / zoomCurrent;
  const hh = originalCameraHeight / 2 / zoomCurrent;
  camera.left = -hw + zoomTargetX; camera.right = hw + zoomTargetX;
  camera.top = hh + zoomTargetY; camera.bottom = -hh + zoomTargetY;
  camera.updateProjectionMatrix();

  if (foodManager) {
    const tickCtx = { ctrl, config, hallOfFame, hofStats, effects, foodManager, foodStats, gizmoList, plankton, updatePlankton };
